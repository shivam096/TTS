...
class Retriever:
    def retrieve(self, retriever_engine, query_str, similarity_threshold=0.3):
        """
        ...
        relevant_nodes = [
            node for node in retrieval_results 
        ...
        is_relevant_query = len(relevant_nodes) > 0 and (
            not hasattr(relevant_nodes[0], 'score') or 
            relevant_nodes[0].score >= SIMILARITY_THRESHOLD
        )
    ...
    def process_query(self, retriever_engine, user_query):
        ...
        if not is_relevant_query:
            return [], "I don't understand how that relates to the database. Please try a query about customer data, sales, or inventory."
        
        # Case 2: Query is relevant but no good matches found
        if not retrieved_text:
            return [], "I understand your query, but I couldn't find relevant information in the database. Could you rephrase or try a different query?"
